...
import { getAllInterviews, getAllSectors } from '@/sanity/lib/queries';
import { urlFor } from '@/sanity/lib/image';
import Link from 'next/link';
import Image from 'next/image';
import { useUser } from '@auth0/nextjs-auth0';
import { getInterviewUrl, getPrimaryRegion, getPrimarySector } from '@/hooks/useInterviews';
...
export default function InterviewsPage() {
  ...
  // Get unique years from interviews, filtering out undefined/null and sorting descending
  const years = Array.from(
    new Set(
      allInterviews
        .map(interview => interview.year)
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [interviewsData, sectorsData] = await Promise.all([
        ...
        setInterviews(interviewsData.slice(0, 6)); // Show first 6 interviews by default
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (selectedSector !== 'all') {
      filtered = filtered.filter(interview => 
        interview.sectors?.some(sector => sector._id === selectedSector)
      );
    }

    if (selectedYear !== 'all') {
      filtered = filtered.filter(interview => interview.year === parseInt(selectedYear));
    }

    setInterviews(filtered.slice(0, showMore ? filtered.length : 6));
  }, [selectedSector, selectedYear, showMore, allInterviews]);
  ...
  return (
    <div className="min-h-screen bg-white poppins-thin">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-16">
          <h2 className="text-4xl font-bold text-[#1E212A] mb-2">Interviews</h2>
          ...
          <div className="lg:w-3/4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              {interviews.map((interview: any) => (
                <Link href={getInterviewUrl(interview)} key={interview._id}>
                    ...
                    <div className="p-4 bg-light-gray h-[200px]">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-gray-500 text-sm">
                          {interview.roleAtTime || interview.interviewee?.role || 'Interview'}
                        </span>
              ...
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                <div className="bg-white p-6 rounded-lg shadow-xl max-w-md mx-4">
                  ...
                  <p className="text-gray-600 mb-6">
                    Please log in to view more interviews and access premium content.
                  </p>
